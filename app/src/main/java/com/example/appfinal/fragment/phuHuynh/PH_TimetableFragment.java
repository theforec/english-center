...
public class PH_TimetableFragment extends Fragment {
    private TableLayout timetable;
    private ArrayList<BuoiHoc> arrBuoiHoc;
    private String ten_lop;
    public static Handler handler;

    public void setTenLop(String lop) {
        this.ten_lop = lop;
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.user_ph_fragment_timetable, container, false);
        init(view);

        handler = new Handler(new Handler.Callback() {
            @Override
            public boolean handleMessage(@NonNull Message msg) {
                if (msg.what == BuoiHocDAO.GET_BUOIHOC_PH) {
                    arrBuoiHoc = msg.getData().getParcelableArrayList("buoihoc");
                    initTable();
                }
                return false;
            }
    ...
    private void init(View view) {
        ...
        buoiHocDAO.getBuoiHocPH(ten_lop);
        timetable = view.findViewById(R.id.timetable_ph);
    }
    ...
    private void initTable() {
        ...
        int height;
        int height_nor = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics());
        ...
        for (int i = 0; i < soDong; i++) {
            ...
            for (int j = 0; j < soCot; j++) {
                ...
                for (int k = 0; k < arrBuoiHoc.size(); k++) {
                    ...
                    if (j + 2 == thu && i == buoi) label.setText(time);
                }
